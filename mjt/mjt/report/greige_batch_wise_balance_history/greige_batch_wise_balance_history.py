...
def get_columns(filters):
	...
	columns = [_("Item") + ":Link/Item:100"] + [_("Item Name") + "::150"] + [_("Description") + "::150"] + \
		...
		[_("UOM") + "::90"] + [_("Party Code") + ":Data:100"] + [_("Party Name") + ":Data:100"] + \
...
def get_conditions(filters):
	...
	if filters.get("batch_no"):
		conditions += " and batch_no like '{0}'".format(filters.get("batch_no"))
	if filters.get("design_no"):
		conditions += " and design_no like '{0}'".format(filters.get("design_no"))
...
def get_stock_ledger_entries(filters):
	...
	return frappe.db.sql("""
		select item_code, design_no, batch_no, warehouse, posting_date, sum(actual_qty) as actual_qty
