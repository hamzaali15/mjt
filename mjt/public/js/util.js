...
erpnext.SerialNoBatchSelector = Class.extend({
	init: function(opts, show_dialog) {
		...
		let d = this.item;
		this.has_batch = 1; this.has_serial_no = 0;
		...
		let fields = [
			...
			},
			{
				...
				onchange: function(e) {
					...
					frappe.call({
						method: 'mjt.mjt.util.get_batch_list',
						args: {
							customer: me.frm.doc.customer,
							item_code: me.item_code,
							warehouse: typeof me.warehouse_details.name == "string" ? me.warehouse_details.name : null,
						},
						callback: (r) => {
							if(r.message){
								for (const d of r.message){
									batches.df.data.push({
										'batch_no': d[0],
										'design_no': d[1],
										'available_qty': d[2],
										'selected_qty': 0
	...
	map_row_values: function(row, values, number, qty_field, warehouse) {
		...
		if (values['design_no'])
		{
			row.design_no = values['design_no'];
		}
				...
				fields: [
					{
						...
						get_query: function () {
							return {
								...
								query: 'mjt.mjt.util.get_batch_no'
							};
						},
						change: function () {
							...
							if(batch_no){
								frappe.db.get_value('Batch', {'name': batch_no}, 'design_no', (r) => {
									if (r && r.design_no) {
										this.grid_row.on_grid_fields_dict.design_no.set_value(r.design_no || '');
									}
								});
							}
							else{
								this.grid_row.on_grid_fields_dict.design_no.set_value('');
							}
